...
class CUB200(torch.utils.data.Dataset):
    ...
    def __len__(self):
        """Return the length of the dataset. The training dataset if it exists else shows the testing dataset."""
        if self._train:
    ...
    def _get_file_list(self, train=True):
        ...
        # Ascertain that there is an images folder after the root folder
        image_path = self._root + '/images/'
        # Image names and their respective folders
        ...
        # Load the bird body part labels
        id2part = np.genfromtxt(self._root + '/parts/part_locs.txt', dtype=float)
        # Load the bounding boxes that represent ground truth
        id2box = np.genfromtxt(self._root + '/bounding_boxes.txt', dtype=float)

        # Training data along with the corresponding labels
        train_data = []
        train_labels = []
        train_parts = []
        train_boxes = []

        # Testing data along with the corresponding labels
        test_data = []
        test_labels = []
        test_parts = []
        test_boxes = []
